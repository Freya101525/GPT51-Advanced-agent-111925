Below is a complete Streamlit implementation that mirrors and extends your original React design, plus an `agents.yaml` example.

Key points covered:

- Streamlit single-page app suitable for Hugging Face Spaces.
- Reads `agents.yaml` for agent definitions; user can edit agents and download updated YAML.
- Multi-provider support: Gemini, OpenAI, Anthropic, XAI.
- API keys read from environment if available; otherwise user can input keys (env keys are never shown).
- Users can choose provider + model per agent, and adjust prompts/parameters before execution.
- Vision OCR for PDFs/images (Gemini/OpenAI) with page-range selection.
- All original features conceptually preserved: upload/OCR, preview/edit, agent swarm config, pipeline execution, analytics dashboard, quick notes with AI formatting & PDF download.
- “Wow” UI:  
  - 20 flower-based themes  
  - Light / dark toggle  
  - English / Traditional Chinese toggle  
  - Status timeline for pipeline execution  
  - Interactive dashboard with charts

---

## 1. `agents.yaml` (top-level file)
...
agents:
  - id: safety_checker
    name: Safety Checker
    description: Check safety endpoints, contraindications and major warnings.
    provider: gemini
    model: gemini-2.5-flash
    temperature: 0.2
    max_tokens: 2048
    system_prompt: |
      You are a meticulous TFDA medical safety reviewer.
      Focus on safety issues, contraindications, and drug interactions.
      Be concise and structured in your findings.
    user_prompt: |
      Please analyze the following TFDA-related document and list:
      - Major safety concerns
      - Contraindications
      - Important warnings and precautions

  - id: dose_consistency
    name: Dose Consistency Auditor
    description: Check consistency of dosage, indications and administration.
    provider: openai
    model: gpt-4o-mini
    temperature: 0.3
    max_tokens: 2048
    system_prompt: |
      You are an expert regulatory reviewer focused on dosage and administration.
      You ensure consistency across all sections of the document.
    user_prompt: |
      Review the text and summarize:
      - Dosage recommendations
      - Administration routes
      - Any inconsistencies or missing information

  - id: summary_report
    name: Regulatory Summary Writer
    description: Generate an executive summary of the document for TFDA reviewers.
    provider: anthropic
    model: claude-3-5-sonnet-latest
    temperature: 0.5
    max_tokens: 4096
    system_prompt: |
      You are a senior TFDA regulatory writer.
      Write concise, well-structured summaries for busy reviewers.
    user_prompt: |
      Based on the following TFDA-related content and any prior analysis,
      generate a clear, concise regulatory summary with headings.
```

You can freely expand this list; the UI will adapt automatically.

---

## 2. `app.py` (Streamlit main app)
...
## 3. Twenty follow‑up questions

1. Would you like OCR to also support Anthropic and xAI vision models, or is Gemini/OpenAI vision sufficient for your workflow?
2. Do you want the OCR step to preserve layout (tables, columns) more aggressively, or is plain linear text acceptable?
3. Should agent chaining always use the previous agent’s output as input, or do you need branching/parallel pipelines and merging?
4. Do you want per-agent control over whether they read the original OCR text, the previous agent’s output, or both?
5. Is it important for you to log and export intermediate inputs/outputs as separate files (e.g., JSONL for audit trails)?
6. Would you like role-based presets (e.g., “Reviewer”, “Writer”, “Data Extractor”) that auto-fill system/user prompts for new agents?
7. Should the app support a “dry run” mode that estimates token usage and cost before actually calling APIs?
8. How important is real-time streaming of model outputs (token-by-token) versus receiving the full response at once?
9. Do you need per-provider timeout/retry controls to improve robustness on unstable network/API conditions?
10. Would you like to add a comparison view to see outputs from different providers/models side-by-side for the same agent?
11. Are there specific TFDA templates (e.g., CTD sections, safety report formats) you want as one-click output formats?
12. Should the analytics dashboard include cost estimation by provider/model using your own price table?
13. Do you want to persist previous runs (logs) across sessions in a database (e.g., SQLite) instead of only in memory?
14. Would integration with object storage (e.g., S3, HF datasets) for uploaded documents and outputs be valuable for you?
15. Do you need user authentication or multi-user workspaces within the same Hugging Face Space?
16. Should the language toggle also translate AI outputs where possible, or only the UI labels as it currently does?
17. Are there specific accessibility requirements (font size, high-contrast mode) you’d like built into the “wow” UI?
18. Would you like additional theme controls (custom color pickers, logo upload) beyond the 20 flower presets?
19. Should the notes panel support rich text/Markdown live preview, or is plain-text editing enough?
20. Do you want scheduled or batch processing (e.g., nightly processing of a folder of PDFs) integrated into this interface?
