...
class LLMRouter:
    ...
    def generate_text(self, model_name: str, messages: List[Dict], params: Dict) -> Tuple[str, Dict, str]:
        ...
        try:
            if provider == "openai":
                ...
                return self._openai_chat(model_name, messages, params), {
                    "total_tokens": self._estimate_tokens(messages)
                }, "OpenAI"
            elif provider == "gemini":
                if not self._gemini_ready:
                    raise Exception("Gemini API not configured")
                return self._gemini_chat(model_name, messages, params), {
                    "total_tokens": self._estimate_tokens(messages)
                }, "Gemini"
            elif provider == "grok":
                if not self._xai_client:
                    raise Exception("Grok API not configured")
                return self._grok_chat(model_name, messages, params), {
                    "total_tokens": self._estimate_tokens(messages)
                }, "Grok"
            elif provider == "anthropic":
                if not self._anthropic_client:
                    raise Exception("Anthropic API not configured")
                return self._anthropic_chat(model_name, messages, params), {
                    "total_tokens": self._estimate_tokens(messages)
                }, "Anthropic"
            else:
    ...
    def _anthropic_chat(self, model: str, messages: List, params: Dict) -> str:
        ...
        if not self._anthropic_client:
            raise Exception(
                "Anthropic API not configured. Please add ANTHROPIC_API_KEY to environment variables."
            )
...
def extract_text_python(
    pdf_bytes: bytes, selected_pages: List[int], ocr_language: str = "english"
) -> str:
    text_parts = []
...
with tab1:
    ...
    if st.button(t["start_ocr"], type="primary", use_container_width=True):
        ...
        page_imgs = st.session_state.page_images
        if not page_imgs:
            st.error("⚠️ No pages rendered. Please upload a PDF first.")
        else:
            ...
            if selected:
                ...
                st.warning("⚠️ No valid pages selected.")
    ...
    if not st.session_state.ocr_text.strip():
        ...
        for i in range(st.session_state.selected_agent_count):
            ...
            with st.expander("📥 Input (editable)", expanded=(i == 0)):
                default_input = (
                    st.session_state.ocr_text
                    if i == 0 and not st.session_state.agent_outputs[i]["input"]
                    else st.session_state.agent_outputs[i]["input"]
                )
                ...
                if st.button(f"▶️ Execute Agent {i+1}", key=f"run_{i}", type="primary"):
                    with st.spinner(f"Agent {i+1} processing..."):
                        ...
                        messages = [
                            ...
                            {
                                "role": "user",
                                "content": f"{agent.get('user_prompt', '')}\n\n"
                                f"{st.session_state.agent_outputs[i]['input']}",
                            },
                        ]
                        params = {
                        ...
                        try:
                            ...
                            st.session_state.agent_outputs[i]["tokens"] = usage.get(
                                "total_tokens", 0
                            )
                            st.session_state.agent_outputs[i]["provider"] = provider
                            st.session_state.agent_outputs[i]["model"] = agent.get(
                                "model", ""
                            )
            ...
            with col_pass:
                if i < st.session_state.selected_agent_count - 1:
                    if st.button(f"➡️ Pass to Agent {i+2}", key=f"pass_{i}"):
                        st.session_state.agent_outputs[i + 1]["input"] = (
            ...
            if output_text:
                ...
                with col_m1:
                    st.markdown(
                        ...
                        f'{st.session_state.agent_outputs[i]["time"]:.2f}s</div>'
                ...
                with col_m2:
                    st.markdown(
                        f'<div class="metric-card"><div class="metric-value">'
                        f'{st.session_state.agent_outputs[i]["tokens"]}</div>'
                ...
                with col_m3:
                    st.markdown(
                        f'<div class="metric-card"><div class="metric-value">'
                        f'{st.session_state.agent_outputs[i]["provider"]}</div>'
                        f'<div class="metric-label">Provider</div></div>',
                        unsafe_allow_html=True,
                    )
                st.text_area(
                    f"Agent {i+1} Output",
            ...
            if st.button("📥 Download JSON", use_container_width=True):
                ...
                payload = {
                    ...
                    "agents": st.session_state.agents_config[
                        : st.session_state.selected_agent_count
                    ],
        ...
        with col_m:
            if st.button("📄 Download Markdown Report", use_container_width=True):
                report = "# FDA Document Analysis Report\n\n"
                report += (
                ...
                for i in range(st.session_state.selected_agent_count):
                    ...
                    report += (
                        f"**Model:** {st.session_state.agent_outputs[i]['model']}\n\n"
                    )
                    report += (
                        f"**Provider:** "
                        f"{st.session_state.agent_outputs[i]['provider']}\n\n"
                    )
                    report += (
                        f"**Processing Time:** "
                        f"{st.session_state.agent_outputs[i]['time']:.2f}s\n\n"
                    )
                    report += (
                        f"### Output\n\n"
                        f"{st.session_state.agent_outputs[i]['output']}\n\n"
                    )
                    report += "---\n\n"
        ...
        with col_r:
            restore_file = st.file_uploader(
                "📤 Restore Session JSON", type=["json"], key="restore"
            )
            if restore_file:
                import json
                ...
                st.session_state.selected_agent_count = len(
                    st.session_state.agents_config
                )
...
with tab5:
    ...
    if not st.session_state.run_metrics:
        ...
        with col3:
            ...
            st.markdown(
                f'<div class="metric-card"><div class="metric-value">'
                f"{avg_latency:.2f}s</div>"
                f'<div class="metric-label">Avg Latency</div></div>',
                unsafe_allow_html=True,
            )
        with col4:
            agents_run = len(df)
            st.markdown(
                f'<div class="metric-card"><div class="metric-value">'
                f"{agents_run}</div>"
                f'<div class="metric-label">Agents Run</div></div>',
                unsafe_allow_html=True,
            )
        ...
        try:
            ...
            for i, rec in enumerate(df.to_dict("records")):
                label = (
                    f"{i+1}. {rec['agent']}\\n"
                    f"{rec['provider']}\\n"
                    f"{rec['latency']:.2f}s | {rec['tokens']} tok"
                )
...
with tab6:
    ...
    st.info(
        "在這裡編輯您的審查筆記。支援 Markdown 和 HTML 顏色標籤，"
        "例如 <span style='color:blue'>藍色文字</span>。"
        "筆記會自動儲存於會話中。"
    )
...
<div style="text-align: center; padding: 2rem; opacity: 0.7;"> 
    ...
    <p style="font-size: 0.8rem;">© 2024 • Theme: {st.session_state.theme}</p>
</div>
""",
    unsafe_allow_html=True,
)
